SUBROUTINE compute_qgpv_direct_inv(nlon, nlat, nlev, kmax, jd, uq, vq, tq, plev, zlev, height, &
        aa, omega, dz, hh, rr, cp, &
        pv, avort, statn, stats, tn0, ts0)


  INTEGER, INTENT(IN) :: nlon, nlat, nlev, kmax, jd
  REAL, INTENT(IN) :: uq(nlon,nlat,kmax), vq(nlon,nlat,kmax), tq(nlon,nlat,kmax), plev(nlev), zlev(nlev), height(kmax)
  REAL, INTENT(in) :: aa, omega, dz,hh, rr, cp
  REAL, INTENT(out) :: pv(nlon,nlat,kmax), avort(nlon,nlat,kmax)
  REAL, INTENT(out) :: statn(kmax), stats(kmax), tn0(kmax), ts0(kmax)
...
! Output: st, uq, vq, tq
! Input: uu, vv, tt (raw temperature), tq (potential temperature), height, plev, zlev
