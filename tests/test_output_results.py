...
def test_expected_value_check_nh18(u_field, v_field, t_field, files_with_expected_values_nh18):
    ...
    qgfield = QGField(
        ...
        tol=tol, rjac=rjac, scale_height=hh, cp=cp, dry_gas_constant=rr,
    ...
    assert np.allclose(qgfield.adv_flux_f1[1:-1, 1:-1], files_with_expected_values_nh18.adv_flux_f1.values[1:-1, 1:-1], rtol), \
        mismatch.format("adv_flux_f1")
    assert np.allclose(qgfield.adv_flux_f2[1:-1, 1:-1], files_with_expected_values_nh18.adv_flux_f2.values[1:-1, 1:-1], rtol), \
        mismatch.format("adv_flux_f2")
    assert np.allclose(qgfield.adv_flux_f3[1:-1, 1:-1], files_with_expected_values_nh18.adv_flux_f3.values[1:-1, 1:-1], rtol), \
        mismatch.format("adv_flux_f3")
    assert np.allclose(qgfield.meridional_heat_flux[1:-1, 1:-1],
                       files_with_expected_values_nh18.meridional_heat_flux.values[1:-1, 1:-1], rtol), \
        mismatch.format("meridional_heat_flux")
    assert np.allclose(qgfield.convergence_zonal_advective_flux[1:-1, 1:-1],
                       files_with_expected_values_nh18.convergence_zonal_advective_flux.values[1:-1, 1:-1], rtol), \
        mismatch.format("convergence_zonal_advective_flux")
    assert np.allclose(qgfield.divergence_eddy_momentum_flux[1:-1, 1:-1],
                       files_with_expected_values_nh18.divergence_eddy_momentum_flux.values[1:-1, 1:-1], rtol), \
        mismatch.format("divergence_eddy_momentum_flux")
