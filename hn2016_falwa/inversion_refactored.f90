SUBROUTINE direct_solver_b4_lu_fact(tjk,sjk,qref,ubar,tbar,fawa,ckref,statn,tn0,imax,JMAX,KMAX,nd,jb,jd,&
        a, om, dz, h, rr, cp, &
        tref)
...
!     USE mkl95_BLAS, ONLY: GEMM,GEMV
      ...
      REAL, INTENT(in) :: tjk(jd-2,kmax-1),sjk(jd-2,jd-2,kmax-1),qref(jd,kmax), ubar(nd,kmax), &
              tbar(nd,kmax),fawa(nd,kmax),ckref(nd,kmax),statn(kmax),tn0(kmax)
      REAL, INTENT(in) :: a, om, dz,h, rr, cp
      REAL, INTENT(out) :: tref(jd,kmax)

!   **** take QG analysis and compute Q_ref and invert for 
!   U_ref & Theta_ref for NH ***
        ...
        !common /bdray/ stats(kmax),ts0(kmax)
        !common /crray/ qn(nnd),an(nnd),aan(nnd)
        real :: tb(kmax),tg(kmax)
        !common /drray/ cn(nnd),ccn(nnd),cbar(nd,kmax)
        real :: alat(nd),phi(nd),z(kmax)
        real :: tj(jd-2),rj(jd-2)
        real :: qjj(jd-2,jd-2),cjj(jd-2,jd-2),qjj_k(jd-2,jd-2,kmax),djj_k(jd-2,jd-2,kmax)
        real :: xjj(jd-2,jd-2),yj(jd-2)
        real :: djj(jd-2,jd-2),sjj(jd-2,jd-2)
        real :: pjk(jd-2,kmax),pj(jd-2)
        real :: u(jd,kmax),cref(nd,kmax)
        !real :: fawa(nd,kmax)
        !real :: qbar(nd,kmax)
        integer :: md(12),ipiv(jd-2),nnd

!        character*34 fn,fn0,fn1
!        character*33 fu
!        character*33 ft
...
!        do n = 1,1
!         fn = '/data/nnn/ERA5/'//fy//'/'//fy//fn2(n)//'QGPV'
!         fu = '/data/nnn/ERA5/'//fy//'/'//fy//fn2(n)//'QGU'
!         ft = '/data/nnn/ERA5/'//fy//'/'//fy//fn2(n)//'QGT'
!         fr = '/data/nnn/ERA5/'//fy//'/'//fy//fn2(n)//'QGREF'
!         write(6,*) fn,md(n)
!        open(35,file =fn,  &
